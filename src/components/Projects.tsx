...
export default function Projects() {
  return (
    <section id="proyek" className="scroll-mt-20 border-t border-border">
      <div className="mx-auto max-w-5xl px-5 py-20">
        <h2 className="text-2xl font-bold tracking-tight sm:text-3xl">
          Proyek yang Pernah <span className="text-gradient">Saya Kerjakan</span>
        </h2>
        <p className="mt-3 max-w-xl text-muted">
          Berikut sejumlah website yang sudah saya bangun dan rilis ke publik.
        </p>
        ...
        <div className="mt-10 grid gap-5 sm:grid-cols-2">
          {projects.map((p) => (
            <article
              key={p.name}
              className="card-hover flex flex-col rounded-xl border border-border bg-surface p-6"
            >
